...
```
flowchart TD
    A[发现定价偏差<br>（如看涨+现金 ≠ 看跌+股票）] --> B[确定套利组合<br>（识别便宜的一方和昂贵的一方）]
    
    B --> C[通过电子交易系统<br>同时下达两笔独立订单]
    
    C --> D1[订单1<br>在新加坡交易所<br>买入被低估的期权/资产]
    C --> D2[订单2<br>在日本交易所<br>卖出被高估的期权/资产]
    
    D1 --> E[新加坡交易所CCP]
    D2 --> F[日本交易所CCP]
    
    E & F --> G[两笔交易独立清算与结算]
    G --> H[套利者赚取价差<br>同时市场价差消失]
